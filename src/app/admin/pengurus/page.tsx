...
type Item = { id: number; name: string; position: string; photoUrl: string | null; contact: string | null; description: string | null; displayOrder: number };
...
export default function AdminPengurusPage() {
  ...
  const [form, setForm] = useState({ name: "", position: "", contact: "", description: "", displayOrder: 0 });
  ...
  function openAdd() {
    setEditing(null);
    setForm({ name: "", position: "", contact: "", description: "", displayOrder: 0 });
    setModal(true);
  }

  function openEdit(item: Item) {
    setEditing(item);
    setForm({ name: item.name, position: item.position, contact: item.contact ?? "", description: item.description ?? "", displayOrder: item.displayOrder });
    setModal(true);
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const url = editing ? `/api/pengurus/${editing.id}` : "/api/pengurus";
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/pengurus/${deleteTarget.id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) { fetchData(); toast.success("Pengurus berhasil dihapus"); setDeleteTarget(null); }
      else toast.error(json.message || "Gagal menghapus pengurus");
    } catch {
    ...
    <div className="p-4 md:p-8">
      <AnimateIn>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
          <div>
            <h1 className="font-display font-extrabold text-3xl md:text-4xl text-brown uppercase tracking-tight">Kelola Pengurus</h1>
            <p className="font-sans text-sm md:text-base text-brown-light mt-1">Atur data pengurus kelas</p>
          </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit Pengurus" : "Tambah Pengurus"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="font-serif font-bold text-sm text-brown block mb-1">Jabatan</label>
            <input value={form.position} onChange={e => setForm({ ...form, position: e.target.value })} className="w-full border-2 border-brown p-3 font-sans text-sm bg-cream outline-none focus:shadow-[4px_4px_0_0_#1f1c0b] transition-shadow" required />
          </div>
          <div>
          ...
          <div>
            <label className="font-serif font-bold text-sm text-brown block mb-1">Urutan Tampil</label>
            <input type="number" value={form.displayOrder} onChange={e => setForm({ ...form, displayOrder: Number(e.target.value) })} className="w-full border-2 border-brown p-3 font-sans text-sm bg-cream outline-none focus:shadow-[4px_4px_0_0_#1f1c0b] transition-shadow" />
          </div>
